...
-- 兼容已有数据库的增量升级（重复执行时由 continue-on-error 忽略）
ALTER TABLE sys_user ADD COLUMN profile VARCHAR(255) DEFAULT NULL COMMENT '个人简介';
ALTER TABLE busi_exchange_order ADD COLUMN initiator_confirmed TINYINT NOT NULL DEFAULT 0 COMMENT '发起方是否确认完成';
ALTER TABLE busi_exchange_order ADD COLUMN target_confirmed TINYINT NOT NULL DEFAULT 0 COMMENT '接收方是否确认完成';
